...
1. The main process hides any prior overlays and grabs each display's frozen desktop. On Windows this is the native FP16 helper (D-015), which runs as a persistent background process warmed at launch so a capture pays no device/duplication setup (D-017); a failed or unverified Windows frame aborts selection with a retry message. Other platforms use `desktopCapturer`. The helper captures the requested displays in one batch.
...
3. Each renderer decodes the desktop image, paints it to the canvas, waits through two animation frames, and acknowledges `capture:ready`. The main process then reveals that overlay by raising its opacity, immediately and with no delay; an unpainted or half-shown full-screen window is never visible (D-010, D-011).
   The screenshot editor coalesces rapid pointer moves into one repaint and filler-scene update per
   animation frame. Pointer-up and exports continue through the ordinary full render path.
...
That divisor is the whole frame's exposure, so a screenshot requires a current Windows SDR-white measurement for every FP16 frame. The display-configuration query is retried; if it still fails, the native helper rejects the screenshot. `IDXGIFactory::IsCurrent` is checked before and after warm acquisitions, and Electron lock, unlock, and resume events clear the helper's duplication, readback, and color caches. HDR state and white level are read after acquiring the frame to avoid mixing pre- and post-transition metadata. The picker retains its short-latency cached/guessed white-level behavior for live sampling. A failed or unverified Windows screenshot never falls back to Chromium's 8-bit source, which can clip HDR pixels before Capturo sees them. `CAPTURO_TIMING=1` names the successful helper's format, HDR state, white level, and source per display; failures log the native stage and show a retryable error.

Windows native screenshots cross into main as temporary, UUID-named PNGs in the OS temp directory.
Main reads and removes them after transfer. `capture-temp.ts` cleans up only Capturo's matching PNGs
older than one hour on a later launch, covering interrupted captures without touching an active
capture or unrelated files. This is a local disk transfer, not a claim that unsaved pixels never
touch storage.
...
The source of truth is an in-memory settings object in the main process, validated by `normalizeSettings` in `src/shared/settings.ts` and persisted to `settings.json` under `app.getPath('userData')`. This is the only settings file Capturo writes, and it holds no captured pixels, only the global open-on-startup/update-check preferences and last-check timestamp, capture format/quality/notification/shortcut preferences, GIF frame-rate/quality/pre-timer/frame-count-visibility/shortcut preferences, and the colour picker's shortcut, copy-on-pick switch, and clipboard format. Capture format and quality affect saved files only; the clipboard stays a lossless bitmap, so they are applied in the main process at save time and never cross into the renderer. The three shortcut recorders map the full Electron accelerator key set and allow every non-modifier key bare or in a chord; only modifier-only and unsupported physical keys are rejected before IPC. Rebinding capture, GIF, or colour picker submits that exact accelerator to `globalShortcut`, then rebuilds the tray menu. If the OS refuses an exclusively held or already registered key, Capturo rolls back to the previous working shortcut because no application can force the OS to deliver it. See D-040. A settings file written before a section existed gains it whole from `normalizeSettings` rather than leaving the feature unconfigured.
...
- `sandbox: true`
- app windows deny renderer-created windows and page navigation; only main loads their local pages
- no remote content
- no telemetry or captured-data network requests; OCR is local, and optional version checks contact only GitHub Releases
- native operations use explicit IPC handlers; Settings actions and capture-session operations
  require their owning main frame
- the display-media handler grants video without audio only to the active GIF or panoramic control
  window and rechecks ownership after source enumeration. Multi-monitor selection requires an exact
  display id; if the OS omits ids on a one-monitor system, its sole screen source is unambiguous
