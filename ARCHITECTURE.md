...
That divisor is the whole frame's exposure, so it is resolved rather than guessed. Windows reports the SDR white level through a pair of display-configuration calls whose result can change between them, and the query is retried before it is treated as unknown; an implausible level is rejected. A level Windows will not report falls back to the last one measured for that output, and only a display never measured at all reaches the compiled-in guess. Each capture reports which of the three it used, because a guessed level lifts every pixel and clips the highlights, which is indistinguishable from the tone mapping having failed. A capture the helper cannot serve at all falls back to Chromium's 8-bit path, which cannot tone map HDR; main logs that rather than letting it pass as an ordinary image, and `CAPTURO_TIMING=1` names the format, HDR state, white level and its source per display.
