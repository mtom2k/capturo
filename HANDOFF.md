...
- Native HDR capture helper: `native/capturo-capture/main.cpp` (one-shot `--output` mode plus a persistent serve mode). Its lifecycle — spawn, warm at launch, batch request with timeout, restart, kill on quit — is `src/main/capture-helper.ts`. The same serialized serve protocol owns Windows DWM border suppression and animated-GIF file copy through `CF_HDROP`; keep those native operations out of the sandboxed renderer. See D-015, D-017, D-021, and D-023.
