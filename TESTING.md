...
13. **Color picker.** Open **Color picker** from the tray menu and confirm it sits directly below **New GIF**.
    - The system cursor must disappear and a magnifier follow the pointer, showing zoomed pixels with the sampled one outlined and its hex beside it. Check the outline stays visible over both white and black areas.
    - **The crosshair must land on the pointer the instant the picker opens, before the mouse is moved at all.** Park the pointer somewhere distinctive, invoke the picker, and confirm the crosshair is already on that pixel rather than in the middle of the screen. Repeat with the pointer near each screen edge and each corner, where the magnifier flips side: the crosshair must stay on the pixel and the magnifier must never cover it.
...
14. **Color window.** After picking, confirm the value shown is the exact pixel color, not one level off.
